...
START_TEST (test_setBitByNumber)
{
    ...
    ck_assert_int_eq(setBitByNumber(NULL, 8, 1,1), 1);

    ck_assert_int_eq(setBitByNumber(bitMap, 8, 2,1), 2);

    ck_assert_int_eq(setBitByNumber(bitMap, 8, 1,9), 3);

    ck_assert_int_eq(setBitByNumber(bitMap, 8, 1,1), 0);

    int pattern[8] = {1,0,0,0,0,0,0,0};

    ck_assert(areArraysEquals(bitMap, pattern, 8));

    ck_assert_int_eq(setBitByNumber(bitMap, 8, 1,8), 0);
...
START_TEST (test_getBitByNumber)
{
    ...
    ck_assert_int_eq(getBitByNumber(NULL, 8,1), NULL);

    ck_assert_int_eq(getBitByNumber(bitMap, 8,9), NULL);

    ck_assert_int_eq(getBitByNumber(bitMap, 8,3), NULL);

    ck_assert_int_eq(getBitByNumber(bitMap, 8, 1), 0);

    ck_assert_int_eq(getBitByNumber(bitMap, 8, 8), 1);
}
END_TEST
